...
def check_args(file_args):
    ...
    try:
        ...
        print("Usage: create_measurements.sh <positive integer number of records>")
        ...
        with open("./data/measurements.txt", "w", encoding="utf-8") as file:
            for s in range(0, num_rows_to_create // batch_size):
                ...
                if s % progress_step == 0:
                    percent = (s * batch_size) / num_rows_to_create * 100
                    print(
                        f"Progresso: {percent:.2f}% ({s * batch_size} "
                        f"de {num_rows_to_create} registros gerados)"
                    )
