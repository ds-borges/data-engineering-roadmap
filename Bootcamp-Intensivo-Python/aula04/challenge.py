...
while not right_name:
    try:
        ...
        if (
            not informacoes_funcionario["nome"]
            or not informacoes_funcionario["nome"].isalpha()
        ):
...
while not right_bonus:
    try:
        ...
        if not informacoes_funcionario["bonus"]:
            ...
            right_bonus = True
    # Isso acontece se o usuário digitar algo que
    # não seja diferente de número (ex: 'abc').

    except ValueError as e:
        print(f"\nErro: {e}")

    # Cálculo do salário final transformando o valor do bonus
    #  em porcentagem para aaplicação do bonus
